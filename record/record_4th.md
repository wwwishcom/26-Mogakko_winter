...
## 추가 정리 : HTML/CSS와 웹 서비스 구조
HTML/CSS는 단순히 예쁜 화면을 만드는 기술이 아니라

웹 서비스 전체 구조에서 클라이언트(Client) 역할을 담당한다.

즉, 웹 서비스는 크게 다음 구조로 이해할 수 있다.

### 웹 서비스는 "클라이언트 <-> 서버"로 동작한다.
1. 클라이언트(client)
   => 사용자가 보는 화면(브라우저)

  HTML/CSS/JS로 구성됨

  브라우저는 사용자의 클릭/입력에 따라 서버에게 요청(Request)을 보내고,
  서버의 응답(Response)을 받아 화면을 갱신한다.

  **Example**

  로그인 버튼 클릭 → 서버에 로그인 요청

  게시글 조회 → 서버에서 글 데이터 받아오기

  검색 → 서버에서 결과 받아오기

2. 서버(Server)
   => 비즈니스 로직을 수행하고 데이터를 반환하는 역할

   => DB 조회/저장, 인증, 추천, 알림 처리 등
